    ...
    async with message.process(ignore_processed=True, requeue=True):
        try:
            ...
            async with await MongoDb.client.start_session() as session:
                voucher_repository = VoucherRepository(session.client.database)
                campaign_repository = CampaignRepository(session.client.database)
                ...
                async with session.start_transaction():
                    ...
                    if available_campaign is not None:
                        ...
                        await campaign_repository.decrease_voucher_from_campaign(
                            available_campaign._id, session=session
                        )

                await message.ack()
                logger.info("Message processed successfully")
        except Exception as e:
            logger.error(f"Error when processing message: {e.with_traceback()}")
            await message.nack()
